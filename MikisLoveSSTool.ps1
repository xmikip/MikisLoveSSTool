...
function Get-GitHubAsset($Url){if($Url -notmatch 'github\.com/([^/]+)/([^/]+)/releases/latest'){return $null};$api="https://api.github.com/repos/$($Matches[1])/$($Matches[2])/releases/latest";$rel=Invoke-RestMethod $api -Headers @{'User-Agent'='MikisLoveSSTool'} -ErrorAction Stop;$a=$rel.assets|Where-Object{$_.name -match '\.(zip|exe|7z|cmd|bat)$'}|Select-Object -First 1;if($a){return @{Url=$a.browser_download_url;Name=$a.name}};return $null}
...
foreach($cat in $Categories){$tab=[Windows.Controls.TabItem]::new();$tab.Header=$cat;$scroll=[Windows.Controls.ScrollViewer]::new();$scroll.VerticalScrollBarVisibility='Auto';$panel=[Windows.Controls.WrapPanel]::new();$panel.Margin='8';foreach($tool in ($ToolData|Where-Object Category -eq $cat)){
 ...
 $btn.Add_Click({$b=$_.Source;$b.IsEnabled=$false;$tool=$b.Tag;$rs=[RunspaceFactory]::CreateRunspace();$rs.ApartmentState='STA';$rs.Open();$rs.SessionStateProxy.SetVariable('tool',$tool);$rs.SessionStateProxy.SetVariable('window',$window);$rs.SessionStateProxy.SetVariable('btn',$b);$rs.SessionStateProxy.SetVariable('statusTitle',$StatusTitle);$rs.SessionStateProxy.SetVariable('statusSub',$StatusSub);$rs.SessionStateProxy.SetVariable('statusBadge',$StatusBadge);$rs.SessionStateProxy.SetVariable('log',$LogBox);$rs.SessionStateProxy.SetVariable('installDir',$installDir);$ps=[PowerShell]::Create();$ps.Runspace=$rs;$ps.AddScript({function UI($a){$window.Dispatcher.Invoke([Action]$a)};UI({$statusTitle.Text='Working';$statusSub.Text="Launching $($tool.Name)...";$statusBadge.Text='BUSY';$log.AppendText("[$(Get-Date -Format HH:mm:ss)] Starting $($tool.Name)...`r`n")});try{& $using:dummy}catch{}; # execution below is handled by main-process dispatcher
 UI({$btn.IsEnabled=$true})})|Out-Null
   # Use a DispatcherTimer to keep all network/PowerShell execution isolated from click handling.
   $timer=[Windows.Threading.DispatcherTimer]::new();$timer.Interval=[TimeSpan]::FromMilliseconds(80);$timer.Tag=@($b,$tool);$timer.Add_Tick({$tm=$_.Source;$tm.Stop();$x=$tm.Tag;Install-AndLaunch $x[1];$x[0].IsEnabled=$true});$timer.Start()
 })
